...
describe Video do
  ...
  describe "search by title" do
    before :each do
      ...
      @first  = Fabricate(:video, title: "Futurama Pair")
      @second = Fabricate(:video, title: "South Park Pair")
    end
  ...
  describe "average_rating" do
    ...
    it "returns the average rating of all of its reviews when present" do
      jurassic_park = Fabricate(:video, title: "Jurassic Park")

      review_one    = Fabricate(:review, video_id: jurassic_park.id, rating: 5)
      review_two    = Fabricate(:review, video_id: jurassic_park.id, rating: 4)
  ...
  describe "reviews method" do
    it "returns reviews in descending order from created_at" do
      ...
      review_one    = Fabricate(:review, video_id: jurassic_park.id, 
                                      created_at: Time.now.ago(100))
      review_two    = Fabricate(:review, video_id: jurassic_park.id, 
                                      created_at: Time.now.ago(50))
      review_three  = Fabricate(:review, video_id: jurassic_park.id, 
                                      created_at: Time.now)
